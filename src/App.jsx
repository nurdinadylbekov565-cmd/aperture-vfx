...
export default function App() {
  ...
  const handleAuth = async (e) => {
    e.preventDefault();
    setLoading(true);
    const cleanEmail = authEmail.trim(); // Убираем пробелы, которые часто добавляет телефон
    ...
    if (error) {
      alert(error.message === "Invalid login credentials" ? "Invalid email or password. If you are new, click 'Switch to Sign Up' below." : error.message);
    } else {
        ...
        <div className="md:hidden fixed inset-0 z-[90] bg-black pt-24 p-6 space-y-4 animate-in slide-in-from-top duration-300">
          {user ? (
            <>
              ...
              <button onClick={() => { supabase.auth.signOut(); setIsMobileMenuOpen(false); }} className="w-full bg-zinc-900 py-5 rounded-2xl font-black uppercase italic">Sign Out</button>
            </>
          ) : (
        ...
        {isDashboardOpen ? (
          ...
          <div className="mt-6 md:mt-10 space-y-8 animate-in fade-in duration-500">
             ...
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-6">
                  <h3 className="text-lg font-black uppercase italic border-l-4 border-blue-600 pl-4">Showcase Assets</h3>
                ...
                {/* Column 2: Brand Inbox */}
                <div className="space-y-6">
                  <h3 className="text-lg font-black uppercase italic border-l-4 border-red-600 pl-4">Project Requests</h3>
                ...
                {/* Column 3: Sent Applications */}
                <div className="space-y-6">
                  <h3 className="text-lg font-black uppercase italic border-l-4 border-green-600 pl-4">My Proposals</h3>
                  <div className="space-y-3">
                    {userApps.map(a => (
          ...
          <>
            ...
            <section className="mt-20 md:mt-32">
              <h2 className="text-4xl md:text-6xl font-black uppercase italic tracking-tighter mb-10">Open <span className="text-zinc-800 text-outline">Requests</span></h2>
      ...
      {isAuthModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/95 backdrop-blur-xl p-4">
          <div className="bg-[#0a0a0a] w-full max-w-md p-8 md:p-10 rounded-[2.5rem] border border-blue-600/30">
            ...
            <button onClick={() => setAuthMode(authMode === 'login' ? 'signup' : 'login')} className="mt-6 text-zinc-600 text-[10px] uppercase font-black block w-full text-center hover:text-white">Switch to {authMode === 'login' ? 'Sign Up' : 'Login'}</button>
